...
// Try the native "Save As" dialog (Chrome/Edge). Falls back to instant download.
...
export async function makePDF(docType, data){
  ...
  let ly = metaTop + 15;
  (data.client?.lines||[]).forEach(l=>{ doc.text(String(l), M, ly); ly += 12.5; });
  if(data.client?.tel){ doc.text(String(data.client.tel), M, ly); ly += 12.5; }
  if(data.client?.email){ doc.text(String(data.client.email), M, ly); ly += 12.5; }
  ...
  meta.forEach(([k,v])=>{
    ...
    doc.text(k, RIGHT-150, my, {align:'left'});
  ...
  (data.items||[]).forEach(it=>{
    ...
    const rowH = Math.max(descLines.length*11, 13);
    ...
    if(discPct>0){
      // light-blue highlight band across the discount row (like FileMaker)
      doc.setFillColor(219, 234, 254);   // soft light blue
  ...
  if(discTotal>0){
    ...
    doc.text('Total:', tLabelX, y, {align:'left'}); doc.text('$'+money(grossTotal), tValX, y, {align:'right'}); y += 16;
    doc.text('Discount:', tLabelX, y, {align:'left'}); doc.text('-$'+money(discTotal), tValX, y, {align:'right'}); y += 16;
  }
  doc.setFont('helvetica','bold'); doc.setFontSize(11); doc.setTextColor(0,0,0);
  doc.text('Sub Total:', tLabelX, y, {align:'left'}); doc.text('$'+money(data.subtotal), tValX, y, {align:'right'}); y += 18;
  doc.text('G.S.T. / H.S.T.:', tLabelX, y, {align:'left'}); doc.text('$'+money(data.hst), tValX, y, {align:'right'}); y += 8;
  doc.setDrawColor(0,0,0); doc.setLineWidth(1); doc.line(tLabelX, y, RIGHT, y); y += 16;
  doc.setFontSize(13);
  ...
  const fname = (docType==='INVOICE'?'Invoice':docType==='PURCHASE ORDER'?'Purchase Order':'Quotation')+'.pdf';
  const blob = doc.output('blob');
  return await savePdfBlob(blob, fname);   // true if saved, false if cancelled
}
