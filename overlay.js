...
// parent → frame:          {yrt:'requestClose'}  (backdrop / Esc)

let _win = null, _onclose = null;
...
export function openWindow(url, { width = 1240, onClose = null } = {}) {
  ...
  fr.style.cssText = 'width:min(' + width + 'px,96vw);height:min(420px,92vh);border:1px solid #414a5c;border-radius:16px;background:#16181d;box-shadow:0 32px 80px -24px rgba(0,0,0,.9);transition:height .18s ease,width .18s ease;';
  ov.appendChild(fr);
  ...
  _win = { ov, fr, esc };
  function askClose(){ try { fr.contentWindow.postMessage({ yrt: 'requestClose' }, '*'); } catch (e) { closeWindow(false); } }
}
